...
pub fn streamable(attr: TokenStream, item: TokenStream) -> TokenStream {
    let found_crate =
        crate_name("cactus-protocol").expect("cactus-protocol is present in `Cargo.toml`");

    let cactus_protocol = match &found_crate {
    ...
    if let Data::Struct(data) = &mut input.data {
        ...
        if is_message {
            extra_impls.push(quote! {
                impl #cactus_protocol::CactusProtocolMessage for #name_ref {
                    fn msg_type() -> #cactus_protocol::ProtocolMessageTypes {
                        #cactus_protocol::ProtocolMessageTypes::#name_ref
                    }
                }
            });
        }
    ...
    let main_derives = quote! {
        #[derive(cactus_streamable_macro::Streamable, Hash, Debug, Clone, Eq, PartialEq)]
    };
    ...
    // If you're calling the macro from `cactus-protocol`, enable Python bindings and arbitrary conditionally.
        ...
        quote! {
            #[cfg_attr(
                feature = "py-bindings", pyo3::pyclass(#class_attrs), derive(
                    cactus_py_streamable_macro::PyJsonDict,
                    cactus_py_streamable_macro::PyStreamable,
                    cactus_py_streamable_macro::PyGetters
                )
            )]
            #main_derives
...
pub fn cactus_streamable_macro(input: TokenStream) -> TokenStream {
    let found_crate = crate_name("cactus-traits").expect("cactus-traits is present in `Cargo.toml`");
        ...
        Data::Enum(e) => {
            ...
            let ret = quote! {
                impl #crate_name::Streamable for #ident {
                    fn update_digest(&self, digest: &mut cactus_sha2::Sha256) {
                        <u8 as #crate_name::Streamable>::update_digest(&(*self as u8), digest);
                    }
                    fn stream(&self, out: &mut Vec<u8>) -> #crate_name::cactus_error::Result<()> {
                        <u8 as #crate_name::Streamable>::stream(&(*self as u8), out)
                    }
                    fn parse<const TRUSTED: bool>(input: &mut std::io::Cursor<&[u8]>) -> #crate_name::cactus_error::Result<Self> {
                        let v = <u8 as #crate_name::Streamable>::parse::<TRUSTED>(input)?;
                        match &v {
                            #(#values => Ok(Self::#names),)*
                            _ => Err(#crate_name::cactus_error::Error::InvalidEnum),
                        }
                    }
                }
            };
    ...
    if !fnames.is_empty() {
        ...
        let ret = quote! {
            impl #crate_name::Streamable for #ident {
                fn update_digest(&self, _digest: &mut cactus_sha2::Sha256) {}
                fn stream(&self, _out: &mut Vec<u8>) -> #crate_name::cactus_error::Result<()> {
                    Ok(())
                }
                fn parse<const TRUSTED: bool>(_input: &mut std::io::Cursor<&[u8]>) -> #crate_name::cactus_error::Result<Self> {
                    Ok(Self{})
                }
            }
        };
