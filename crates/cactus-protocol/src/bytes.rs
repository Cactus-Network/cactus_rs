...
use cactus_traits::{cactus_error, read_bytes, Streamable};
...
use cactus_traits::{CactusToPython, FromJsonDict, ToJsonDict};
#[cfg(feature = "py-bindings")]
use hex::FromHex;
#[cfg(feature = "py-bindings")]
...
impl Streamable for Bytes {
    ...
    fn stream(&self, out: &mut Vec<u8>) -> cactus_error::Result<()> {
        if self.0.len() > u32::MAX as usize {
            Err(cactus_error::Error::SequenceTooLarge)
        } else {
    ...
    fn parse<const TRUSTED: bool>(input: &mut Cursor<&[u8]>) -> cactus_error::Result<Self> {
...
impl<const N: usize> ToPyObject for BytesImpl<N> {
    fn to_object(&self, py: Python<'_>) -> PyObject {
        CactusToPython::to_python(self, py).unwrap().into()
    }
}
...
#[cfg(feature = "py-bindings")]
impl<const N: usize> CactusToPython for BytesImpl<N> {
...
mod tests {
    ...
    fn test_parse_truncated_len() {
        let buf: &[u8] = &[0, 0, 1];
        from_bytes_fail::<Bytes>(buf, cactus_error::Error::EndOfBuffer);
    }
    ...
    fn test_parse_bytes32() {
        ...
        from_bytes_fail::<Bytes32>(&buf[0..30], cactus_error::Error::EndOfBuffer);
    }
    ...
    fn test_parse_bytes48() {
        ...
        from_bytes_fail::<Bytes48>(&buf[0..47], cactus_error::Error::EndOfBuffer);
    }
