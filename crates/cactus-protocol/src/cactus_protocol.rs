use cactus_streamable_macro::{streamable, Streamable};

use crate::Bytes;

#[cfg(feature = "py-bindings")]
use cactus_py_streamable_macro::{PyJsonDict, PyStreamable};
...
#[cfg(feature = "py-bindings")]
impl cactus_traits::CactusToPython for ProtocolMessageTypes {
...
pub trait CactusProtocolMessage {
...
#[cfg(feature = "py-bindings")]
impl cactus_traits::CactusToPython for NodeType {
    fn to_python<'a>(&self, py: pyo3::Python<'a>) -> pyo3::PyResult<pyo3::Bound<'a, pyo3::PyAny>> {
        Ok(pyo3::IntoPy::into_py(*self, py).bind(py).clone().into_any())
    }
}
