...
pub fn test_streamable<T: Streamable + std::fmt::Debug + PartialEq>(obj: &T) {
    ...
    assert!(T::from_bytes_unchecked(&corrupt_bytes) == Err(cactus_traits::Error::InputTooLarge));

    if !bytes.is_empty() {
        // make sure input too short is an error
        corrupt_bytes.truncate(bytes.len() - 1);
        assert!(T::from_bytes_unchecked(&corrupt_bytes) == Err(cactus_traits::Error::EndOfBuffer));
    }
}
