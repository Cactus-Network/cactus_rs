// This is a throw-away tool to scan a cactus blockchain database for spends that
...
use cactus_tools::{iterate_blocks, visit_spends};
use cactus_traits::streamable::Streamable;

use cactus_bls::G2Element;
use cactus_protocol::{Bytes32, Coin, CoinSpend, Program, SpendBundle};
use cactus_puzzles::singleton::SINGLETON_TOP_LAYER_PUZZLE_HASH;
...
/// Analyze the spends in a cactus blockchain database
#[derive(Parser, Debug)]
        ...
        |height, block, block_refs| {
            ...
            pool.execute(move || {
                ...
                if args.block_generators {
                    let directory = "../cactus-protocol/fuzz/corpus/run-generator";
                    let _ = std::fs::create_dir_all(directory);
                    write(format!("{directory}/{height}.bundle"), generator).expect("write");

                    let directory = "../cactus-protocol/fuzz/corpus/additions-and-removals";
                    let _ = std::fs::create_dir_all(directory);
                    write(format!("{directory}/{height}.bundle"), generator).expect("write");
                ...
                if args.puzzles || args.spend_bundles || args.coin_spends {
                    visit_spends(
                        ...
                        |a, parent_coin_info, amount, puzzle, solution| {
                            ...
                            if run_puzzle {
                                let directory = "../cactus-consensus/fuzz/corpus/run-puzzle";
                                let _ = std::fs::create_dir_all(directory);
                            ...
                            if fast_forward {
                                let directory = "../cactus-consensus/fuzz/corpus/fast-forward";
                                let _ = std::fs::create_dir_all(directory);
                ...
                if args.spend_bundles && !bundle.coin_spends.is_empty() {
                    let directory = "../cactus-protocol/fuzz/corpus/spend-bundle";
                    let _ = std::fs::create_dir_all(directory);
                ...
                if args.coin_spends && !bundle.coin_spends.is_empty() {
                    let directory = "../cactus-consensus/fuzz/corpus/solution-generator";
                    let _ = std::fs::create_dir_all(directory);
                    let mut f =
