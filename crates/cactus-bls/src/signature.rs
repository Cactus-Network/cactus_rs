...
use cactus_traits::{read_bytes, Streamable};
...
#[cfg_attr(
    ...
    derive(cactus_py_streamable_macro::PyStreamable)
)]
...
impl Streamable for Signature {
    ...
    fn stream(&self, out: &mut Vec<u8>) -> cactus_traits::cactus_error::Result<()> {
    ...
    fn parse<const TRUSTED: bool>(
        input: &mut Cursor<&[u8]>,
    ) -> cactus_traits::cactus_error::Result<Self> {
...
mod pybindings {
    ...
    use cactus_traits::{FromJsonDict, ToJsonDict};
...
mod tests {
    ...
    fn test_vector_2_aggregate_of_aggregates() {
        ...
        // "Cactus test vector 2 (Augmented, aggregate of aggregates)"
    ...
    // test cases from zksnark test in cactus_rs
    #[rstest]
