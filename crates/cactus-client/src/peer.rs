...
impl Peer {
        ...
        let response: RespondBlockHeaders =
            self.request_or_reject(body)
                ...
                .map_err(|error: Error<RejectBlockHeaders>| match error {
                    ...
                    Error::Cactus(error) => Error::Cactus(error),
    ...
    where
        T: Streamable + CactusProtocolMessage,
    {
        // Create the message.
        let message = Message {
            msg_type: T::msg_type(),
    ...
    where
        T: Streamable + CactusProtocolMessage,
        R: Streamable + CactusProtocolMessage,
        B: Streamable + CactusProtocolMessage,
    {
    ...
    where
        Response: Streamable + CactusProtocolMessage,
        T: Streamable + CactusProtocolMessage,
    {
        let message = self.request_raw(body).await?;
        let data = message.data.as_ref();
